...
flowchart TD
  ...
  subgraph REPO[Project-Auth-GitOps]
    ...
    subgraph ARGO[argocd/dev]
      ...
      A3[applications/infra/platform.yaml]
      A4[applications/infra/sealed-secrets.yaml]
    end

    subgraph MANIFESTS[apps]
      M1[auth-server/overlays/dev]
      M2[api-server/overlays/dev]
      M3[auth-server/overlays/dev/platform]
    end
  end
  ...
  subgraph CLUSTER[Dev Cluster]
    ...
    N4[namespace: kube-system]
    W1[auth-server]
    W2[api-server]
    W3[postgres]
    W4[keycloak]
    W5[vault]
    W6[sealed-secrets-controller]
  end
...
4. prod 반영은 GitOps repo의 `workflow_dispatch`를 통해 보호된 환경 승인 절차를 거쳐 수동으로 진행합니다.
5. GitOps repo의 `kustomization.yaml`이 변경되면 Argo CD가 이를 감지합니다.
6. Argo CD가 cluster에 실제 배포를 반영합니다.
